using System;
using System.Linq;
using System.Threading.Tasks;
using Microsoft.EntityFrameworkCore;
using ERP.Application.Common.Interfaces;
using ERP.Application.Common.Models;
using ERP.Application.DTOs.Assets;
using ERP.Domain.Entities.Asset;

namespace ERP.Application.Services.Assets;

public class AssetAssignmentService : IAssetAssignmentService
{
    private readonly IApplicationDbContext _context;

    public AssetAssignmentService(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<PagedResponse<AssetAssignmentDto>> GetPagedAsync(PagedRequest request)
    {
        var query = _context.AssetAssignments.AsQueryable();

        if (!string.IsNullOrWhiteSpace(request.SearchTerm))
        {
            // Add custom search logic here if needed
        }

        var totalCount = await query.CountAsync();

        if (!string.IsNullOrWhiteSpace(request.SortBy))
        {
            if (request.SortDescending)
            {
                query = query.OrderByDescending(x => EF.Property<object>(x, request.SortBy));
            }
            else
            {
                query = query.OrderBy(x => EF.Property<object>(x, request.SortBy));
            }
        }

        var items = await query
            .Include(x => x.Asset)
            .Skip((request.Page - 1) * request.PageSize)
            .Take(request.PageSize)
            .ToListAsync();
            
        var dtos = items.Select(x => MapToDto(x)).ToList();

        return new PagedResponse<AssetAssignmentDto>(dtos, totalCount, request.Page, request.PageSize);
    }

    public async Task<AssetAssignmentDto?> GetByIdAsync(long id)
    {
        var entity = await _context.AssetAssignments
            .Include(x => x.Asset)
            .FirstOrDefaultAsync(x => x.Id == id);
            
        if (entity == null) return null;
        
        return MapToDto(entity);
    }

    public async Task<AssetAssignmentDto> CreateAsync(CreateAssetAssignmentDto dto)
    {
        var entity = new AssetAssignment();
        MapToEntity(dto, entity);
        
        _context.AssetAssignments.Add(entity);
        await _context.SaveChangesAsync();
        
        return await GetByIdAsync(entity.Id) ?? MapToDto(entity);
    }

    public async Task<AssetAssignmentDto> UpdateAsync(long id, UpdateAssetAssignmentDto dto)
    {
        var entity = await _context.AssetAssignments.FindAsync(id);
        if (entity == null) throw new Exception("AssetAssignment not found");
        
        MapToEntity(dto, entity);
        
        await _context.SaveChangesAsync();
        return await GetByIdAsync(entity.Id) ?? MapToDto(entity);
    }

    public async Task<bool> DeleteAsync(long id)
    {
        var entity = await _context.AssetAssignments.FindAsync(id);
        if (entity == null) return false;
        
        _context.AssetAssignments.Remove(entity);
        await _context.SaveChangesAsync();
        return true;
    }

    private static AssetAssignmentDto MapToDto(AssetAssignment entity)
    {
        return new AssetAssignmentDto
        {
            Id = entity.Id,
            AssetId = entity.AssetId,
            FromDepartmentId = entity.FromDepartmentId,
            ToDepartmentId = entity.ToDepartmentId,
            FromCustodianId = entity.FromCustodianId,
            ToCustodianId = entity.ToCustodianId,
            FromLocationId = entity.FromLocationId,
            ToLocationId = entity.ToLocationId,
            AssignmentDate = entity.AssignmentDate,
            Reason = entity.Reason,
            ApprovedBy = entity.ApprovedBy,
        };
    }

    private static void MapToEntity(object dto, AssetAssignment entity)
    {
        var props = dto.GetType().GetProperties();
        foreach (var prop in props)
        {
            var entityProp = entity.GetType().GetProperty(prop.Name);
            if (entityProp != null && entityProp.CanWrite)
            {
                entityProp.SetValue(entity, prop.GetValue(dto));
            }
        }
    }
}
